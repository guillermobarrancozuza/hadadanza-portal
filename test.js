    ...
    createApp({
      setup() {
        ...
        async function toggleCalendarSync() {
          ...
          try {
            const res = await fetch(url, {
              ...
              body: JSON.stringify({ artistId: activeArtist.value.id })
            });
            const data = await res.json();
            isSynced.value = data.isSubscribed;
          } catch (err) {
            console.error(err);
          } finally {
        ...
        return {
          ...
          currentDate, currentMonthLabel, sortedEvents, stats, calendarDays, modalOpen, isEditing, form, menuItems, tabs, darkTheme,
          openGlobalView, selectArtist, filterEvents, prevMonth, nextMonth, setToday, openCreateModal, openEditModal, saveEvent, deleteEvent, generateContractPDF, getEventClasses, getEventIcon, formatStatus, getStatusBadgeClass, formatCurrency, formatDate, toggleTheme, isSynced, syncLoading, toggleCalendarSync
        };
      }
    }).mount('#app');
